...
Twist2D Waypoints::nextWaypoint(Twist2D current_pose)
{
    ...
    double x_delta = (waypoints[0].x - current_pose.v_x);
    double y_delta = (waypoints[0].y - current_pose.v_y);
    double angle_req = atan2(y_delta, x_delta);
    
    std::cout<< "x_delta" <<x_delta<<"\n";
    std::cout<< "y_deta" << y_delta<<"\n";

     
    double v_x_world = sqrt(y_delta*y_delta + x_delta * x_delta);

    std::cout<< "v_x_world= " << v_x_world<<"\n";

    //double v_x_body = v_x_world/cos(atan2((waypoints[0].y - current_pose.v_y), (waypoints[0].x - current_pose.v_x)));
    output_twist.v_x = v_x_world;
    output_twist.v_y = 0;
