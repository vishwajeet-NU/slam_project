...
{ 
    ...
    EKF()
    {
      path_pub_map = n.advertise<nav_msgs::Path>("/path_map", 1);
    ...
    Eigen::MatrixXd ekf_predict_mean(Twist2D &body_v, Eigen::MatrixXd mu_t_bar)
    {
...
Eigen::MatrixXd ekf_predict_G(Twist2D &body_v, Eigen::MatrixXd g_t)
{
...
//     map_trans.transform.translation.x = mu_t_bar.coeff(1,0) - odom_x;
//     map_trans.transform.translation.y = mu_t_bar.coeff(2,0) - odom_y;
...
int main(int argc, char** argv)
{
     ...
     EKF turtle;
     ros::param::get("sigma_r",turtle.sigma_r);
     ros::param::get("sigma_theta",turtle.sigma_theta);
     ros::param::get("sigma_landmark",turtle.sigma_landmark);
     ros::param::get("co_var_w",turtle.co_var_w);
     ros::param::get("co_var_v",turtle.co_var_v);
     ros::param::get("r_param",turtle.r_param);

    // ros::Timer timer = n.createTimer(ros::Duration(0.01), &EKF::do_tf, &turtle);
    ...
    ros::Subscriber read_landmarks = n.subscribe("landmarks", 1, &EKF::landmarks,&turtle );
    ros::Subscriber odom_sub = n.subscribe("/odom", 1, &EKF::pose_odom_Callback, &turtle);
      ...
      while(n.ok())
      {
          ...
          if(turtle.land_called && turtle.odom_call)
          {

              if(start)
              {
                  for(int i =0; i<turtle.m_land; i++)
                  {
                      turtle.mu_t_bar.row(2+2*(i+1)-1) << turtle.x_center[i];
                      turtle.mu_t_bar.row(2+2*(i+1)) << turtle.y_center[i];
                  }
            ...
            mu_t_bar = turtle.ekf_predict_mean(body_v,mu_t_bar);
            g_t = turtle.ekf_predict_G(body_v,g_t);
            turtle.ekf_update();
        
            // do_tf 
            turtle.land_called = false;
            turtle.odom_call = false;
